...
def check_versions(str_filename,ffdoc_filename):
	...
	for line in f.readlines():
		...
		if line.startswith("* For use with CGenFF topology and parameter files version"):
			entry = re.split(r'\s+', line.lstrip())
			strver = entry[10]
			print("--Version of CGenFF detected in ",str_filename,":",strver)
	f.close()
...
def _networkx_major_minor(version):
	version = version.split("+", 1)[0]
	parts = version.split(".")
	return float(parts[0] + "." + parts[1])

#check for compatible NetworkX version
if(_networkx_major_minor(nx.__version__) < 2.0):
	...
	if(_networkx_major_minor(nx.__version__) > 2.3):
