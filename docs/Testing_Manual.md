...
### 1.2 Membrane (Optimized)
```bash
mstbx topopsfgen --env membrane --psf lipids.psf --pdb lipids.pdb --z-distance 10 --salt 0.150
```
*   **Verification**: Default Z-padding is now **25Å** (matches updated safety standards). Check that the XY box is a perfect square.
...
```
*   **Verification**: `--dcdfreq` is now in **ps**. 10.0 ps = 5000 steps (at 2fs).
